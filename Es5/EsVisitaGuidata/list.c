...
ItemType *trovaMigliore(int numper, LIST listaGuide) {
    ItemType *migliore = NULL;
    LIST tmp = listaGuide;
    ...
    while (tmp != NULL) {
        ...
        if (it->massimo >= numper && it->minimo <= numper) {
            if (migliore != NULL && ((migliore->massimo - numper) >= (it->massimo - numper))) {
                migliore = it;
            }
            if (migliore == NULL) {
                migliore = it;
...
void PrintItem(ItemType item) {
    /*** esempio ***/
    printf("%s %i %i\n", item.nome, item.minimo, item.massimo);
}
