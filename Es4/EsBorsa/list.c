...
ItemType* FindByNome(LIST l, char* nome) {
    LIST tmp = l;
    /*if (tmp == NULL) {
      return NULL;
    }*/
    while (tmp != NULL && (strcmp(tmp->item.nome, nome) != 0)) {
        tmp = tmp->next;
    }
    if (tmp == NULL) {
    	return NULL;
...
void PrintItem(ItemType item) {
    /*** esempio ***/
    printf("Agente: %s, quantita': %i, prezzo unitario: %i, prezzo minimo: %i\n", item.nome, item.quantita, item.prezzounitario, item.prezzominimo);
}
