...
int itemCompare(ItemType item1, ItemType item2) {
    if (item1.tipo != item2.tipo) {
    	return item1.tipo - item2.tipo;
    }
    
    if (item1.tipo == FORNITORE) {
    	return strcmp(item1.itemf.nome, item2.itemf.nome);
    } else {
    	return strcmp(item1.itemc.nome, item2.itemc.nome);
    }
}
...
LIST FindCentri(LIST centri, ItemType fornitore) {
	LIST output = NewList();
	LIST tmp = centri;
	while (tmp != NULL) {
		if (tmp->item.tipo == CENTRO && tmp->item.itemc.numero_pazienti >= fornitore.itemf.quantita_minima) {
			output = EnqueueLast(output, tmp->item);
		}
		tmp = tmp->next;
	}
	return output;
...
void PrintItem(ItemType item) {
    /*** esempio ***/
    if (item.tipo == FORNITORE) {
    	printf("Nome fornitore: %s, quantita' vaccini: %i, quantita' minima: %i\n", item.itemf.nome, item.itemf.quantita_vaccini, item.itemf.quantita_minima);
    } else if (item.tipo == CENTRO) {
    	printf("Nome centro: %s, numero pazienti: %i\n", item.itemc.nome, item.itemc.numero_pazienti);
    }
}
